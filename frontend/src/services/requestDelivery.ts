...
export async function sendProvisionRequest(
  ...
  if (!hasEmailConfiguration()) {
    return {
      emailSent: false,
      message: formatWhatsAppRequest(customer, items, products),
    }
  }
  await emailjs.send(
    import.meta.env.VITE_EMAILJS_SERVICE_ID,
    import.meta.env.VITE_EMAILJS_TEMPLATE_ID,
    {
      ...
      request_items: formatEmailItems(items, products),
      customer_notes: customer.notes?.trim() || 'None',
      request_timestamp: sentAt.toLocaleString('en-NG', {
        dateStyle: 'full',
        timeStyle: 'short',
      }),
    },
