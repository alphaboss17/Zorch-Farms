...
export function Header() {
  ...
  const submitSearch = (event: FormEvent<HTMLFormElement>) => {
    ...
    navigate(`/categories${value ? `?search=${encodeURIComponent(value)}` : ''}`)
  }
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-line/40 bg-canvas/90 backdrop-blur-md">
      <div className="page-shell flex h-16 items-center justify-between gap-2 sm:gap-4">
        <Link
          to="/"
          className="shrink-0 font-primary text-base sm:text-lg font-extrabold tracking-tight text-forest"
        >
          Zorch Farms
        </Link>
        ...
        <div className="flex items-center gap-1.5 sm:gap-3">
          <Button
            ...
            className="relative shrink-0 px-2.5 sm:px-3.5 font-primary"
          >
            <span>
              <span className="hidden min-[360px]:inline">View </span>Request
            </span>
            {totalItems > 0 && (
